...
out_file <- "/home/ziadbkh/Desktop/NECTAR_W_252_C2/eQTLHap/comparison/results/R_lm-WithCov"
...
row.names(cov) <- cov$ID
cov <- cov[, 2:length(cov)]
...
for (gn_nm_itr in 1:nrow(ge_data))
{
  ...
  for (block_id in 1:nrow(curr_gene_blocks))
  {
    ...
    lm_model <- lm(curr_gene_expre ~ t(cov) + factor(geno_blk));
    res <- anova(lm_model)[2, c("F value","Pr(>F)")];
    ...
    hap <- as.matrix(hap[ colnames(cov), ])
    
    lm_model <- lm(curr_gene_expre ~ t(cov) + hap);
    res <- anova(lm_model)[2, c("F value","Pr(>F)")];
  ...
  for (snp_iter in 1:nrow(curr_block_h1))
  {
    ...
    lm_model <- lm(curr_gene_expre ~ t(cov) + snp)
